...
基于 [VOFA+](https://www.vofa.plus/) 上位机的嵌入式串口调参/波形显示库，支持 `JustFloat`、`Firewater`、`Rawdata` 三种发送协议，以及可自定义的命令帧解析。

## 特性

- **整帧一次发送**：JustFloat 帧拼好后一次发出，小端平台 float 直接 `memcpy` 零转换
- **移植零侵入**：串口发送、命令帧解析均为弱符号，在自己的工程里定义同名函数即可覆盖，无需修改库源码
- **通道数可变**：`vofaSendJustFloat` 按需传入 1~8 个通道
- **解析结果可查**：`vofaCommandParse` 返回解析状态，坏帧不再静默丢弃
- **中断安全**：中断与主循环共享变量均为 `volatile`，接收先整体拷贝再解析
